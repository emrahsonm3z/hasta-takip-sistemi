...
├── router/
│   └── index.tsx            createBrowserRouter: showcase home (/) + /showcase→/ + pathless AppLayout (module routes + 404) (§6)
...
| `showcase` | Tanıtım sayfası / Project showcase         | `docs/en/modules/SHOWCASE.md` |
...
and a 404. The index route `/` is the showcase home (its own `ShowcaseLayout`,
outside the patient `AppLayout` shell); the legacy `/showcase` path redirects to
`/`.
...
- **Aggregation lives in `router/index.tsx`.** It builds the showcase home
  route (`/` → `ShowcaseLayout`) plus its legacy `/showcase` → `/` redirect, and
  a pathless `AppLayout` route (`errorElement={<RouteErrorBoundary/>}`) whose
  children are the spread module arrays and a `*` 404, then calls
...
| `modules/SHOWCASE.md`   | Showcase module detail (standalone route, scroll-spy, live preview) |
...
- **Documented exception**: `modules/SHOWCASE.md` is in the index (§13.2) and the
  Module Index (§4) but **intentionally NOT in the `docsRegistry`** — the showcase
  is the public landing page itself, so its module doc is deliberately not
  surfaced in the in-app docs viewer (`/docs/module-showcase` 404s by design). Do
  not re-add it to the registry.
